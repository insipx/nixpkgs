...
stdenv.mkDerivation (finalAttrs: {
  ...
  # readline's depends on terminal APIs unavailable on iOS.
  ...
  # updateAutotoolsGnuConfigScriptsHook glob. needs to be refreshed manually to include new triples.
  postPatch = ''
  ...
  # The tclsqlite extension links target objects against the build
  # platform's tclConfig.sh, which drags build libc into the target link
  # broken for any cross build (musl, iOS). Native-only.
  ...
  env = {
    NIX_CFLAGS_COMPILE = toString (
      ...
      # ios does not have system available
